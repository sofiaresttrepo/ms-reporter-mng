...
const CONTEXT_NAME = "reporter-mng";
...
module.exports = {
    ...
    //// QUERY ///////
    Query: {
        ReporterMngReportListing(root, args, context) {
            return sendToBackEndHandler$(root, args, context, READ_ROLES, 'query', 'Report', 'ReporterMngReportListing').toPromise();
        },
        ReporterMngReport(root, args, context) {
            return sendToBackEndHandler$(root, args, context, READ_ROLES, 'query', 'Report', 'ReporterMngReport').toPromise();
        }
    },

    //// MUTATIONS ///////
    Mutation: {
        ReporterMngCreateReport(root, args, context) {
            return sendToBackEndHandler$(root, args, context, WRITE_ROLES, 'mutation', 'Report', 'ReporterMngCreateReport').toPromise();
        },
        ReporterMngUpdateReport(root, args, context) {
            return sendToBackEndHandler$(root, args, context, WRITE_ROLES, 'mutation', 'Report', 'ReporterMngUpdateReport').toPromise();
        },
        ReporterMngDeleteReports(root, args, context) {
            return sendToBackEndHandler$(root, args, context, WRITE_ROLES, 'mutation', 'Report', 'ReporterMngDeleteReports').toPromise();
        },
    },

    //// SUBSCRIPTIONS ///////
    Subscription: {
        ReporterMngReportModified: {
            subscribe: withFilter(
                (payload, variables, context, info) => {
                    ...
                    return pubsub.asyncIterator("ReporterMngReportModified");
                },
                (payload, variables, context, info) => {
                    return payload
                        ? (payload.ReporterMngReportModified.id === variables.id) || (variables.id === "ANY")
                        : false;
                }
            )
        }
...
const eventDescriptors = [
    {
        backendEventName: "ReporterMngReportModified",
        gqlSubscriptionName: "ReporterMngReportModified",
